...
classdef PresentMoviesImageClass < manookinlab.protocols.ManookinLabStageProtocol
    
    properties
        amp                             % Output amplifier
        preTime     = 0                 % Leading duration in msec
    ...
    methods
        ...
        function p = createPresentation(obj)
            ...
            % Use first frame for initialization
            scene = stage.builtin.stimuli.Image(obj.imageMatrix{1});
            scene.size = [size(obj.imageMatrix{1},2), size(obj.imageMatrix{1},1)];
            ...
            stimFrames = floor(obj.stimTime*1e-3 * obj.frameRate);
            imgValue = stage.builtin.controllers.PropertyController(scene, ...
                'imageMatrix', @(state) getFrame(obj, state.frame - preF, stimFrames));
            p.addController(imgValue);

            function img = getFrame(obj, frameIdx, stimFrames)
                % Clamp frameIdx to valid range
                if frameIdx < 1 || frameIdx > min(obj.numFrames, stimFrames)
                    img = obj.backgroundImage;
                else
                    img = obj.imageMatrix{frameIdx};
                end
            end
