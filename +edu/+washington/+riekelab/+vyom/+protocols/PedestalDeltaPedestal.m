classdef PedestalDeltaPedestal < manookinlab.protocols.ManookinLabStageProtocol
    properties
...
%                     %@(state)state.time >= 0 && state.time < (obj.preTime + obj.stimTime + obj.tailTime) * 1e-3);
                    ...
                    p.addStimulus(rect);
                    barColor = stage.builtin.controllers.PropertyController(rect, 'color', ...
                        @(state)setFlashContrast(obj, state.time-obj.preTime*1e-3));
                    p.addController(barColor);
                    %barVisible = stage.builtin.controllers.PropertyController(rect, 'visible', ...
                    %@(state)state.time >= obj.preTime * 1e-3 && state.time < (obj.preTime + obj.stimTime) * 1e-3);
                    %barVisible = stage.builtin.controllers.PropertyController(rect, 'visible', ...
          ...
          function c = setFlashContrast(obj, time)
                if time >= obj.preFlashTime * 1e-3 && ...
                     time < (obj.preFlashTime + obj.flashTime) * 1e-3
                    c = obj.stimFlashIntensity;
                else
                    c = obj.intensity;
                end
            end
...
%             p.addController(barVisible);
        ...
        function prepareEpoch(obj, epoch)
            prepareEpoch@manookinlab.protocols.ManookinLabStageProtocol(obj, epoch);
              ...
              if mod(obj.numEpochsCompleted+1,length(testContrasts)*5)==0
                 % After each cycle of test contrasts                            
                 obj.idxFlashContrast = mod(obj.idxFlashContrast, length(flashContrasts))+1;
              end
              if mod(obj.numEpochsCompleted+1,length(testContrasts)*length(flashContrasts)*5)==0
