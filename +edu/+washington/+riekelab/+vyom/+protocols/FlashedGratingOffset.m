...
% classdef FlashedGratingOffset < manookinlab.protocols.ManookinLabStageProtocol%edu.washington.riekelab.protocols.RiekeLabStageProtocol
%     properties
%         preTime = 250 % ms
%         stimTime = 250 % ms
%         tailTime = 500 % ms
...
%         end
%         function prepareRun(obj)
%             prepareRun@manookinlab.protocols.ManookinLabStageProtocol(obj);%edu.washington.riekelab.protocols.RiekeLabStageProtocol(obj);
%             disp('Started prepare run')
% %             obj.showFigure('symphonyui.builtin.figures.ResponseFigure', obj.rig.getDevice(obj.amp));
% % 
% %             obj.showFigure('edu.washington.riekelab.turner.figures.MeanResponseFigure',...
% %                 obj.rig.getDevice(obj.amp),'recordingType',obj.onlineAnalysis,...
...
% % 
% %                 obj.showFigure('edu.washington.riekelab.turner.figures.ModImageVsIntensityFigure', ...
% %                 obj.rig.getDevice(obj.amp),responseDimensions,'recordingType',obj.onlineAnalysis,...
% %                 'preTime',obj.preTime,'stimTime',obj.stimTime,'stimType','grating');
% % 
% % 
%             % Create surround contrast sequence.
% 
%             obj.surroundContrastSequence = obj.surroundContrast;
%             disp('Done prepare run')
%         end
%         function prepareEpoch(obj, epoch)
%             disp('Started prepare epoch')
%             prepareEpoch@manookinlab.protocols.ManookinLabStageProtocol(obj, epoch);%prepareEpoch@edu.washington.riekelab.protocols.RiekeLabStageProtocol(obj, epoch);
%             %device = obj.rig.getDevice(obj.amp);
%             %duration = (obj.preTime + obj.stimTime + obj.tailTime) / 1e3;
%             %epoch.addDirectCurrentStimulus(device, device.background, duration, obj.sampleRate);
%             %epoch.addResponse(device);
...
%             epoch.addParameter('currentSurroundContrast', obj.currentSurroundContrast);
%             epoch.addParameter('currentPhaseOffset', obj.currentPhaseOffset);
%             disp('Done prepare epoch')
%         end
%         function p = createPresentation(obj)  
%             disp('Started creating presentation')
%             canvasSize = obj.rig.getDevice('Stage').getCanvasSize();
%             p = stage.core.Presentation((obj.preTime + obj.stimTime + obj.tailTime) * 1e-3);
%             p.setBackgroundColor(obj.backgroundIntensity);
%             apertureDiameterPix = obj.rig.getDevice('Stage').um2pix(obj.apertureDiameter);
...
%                 disp('bp 3')
%             end
%             disp('Done creating presentation')
%             keyboard;
%         end
%         function tf = shouldContinuePreparingEpochs(obj)
%             disp('Started scpe');
